...
class UriService::Client
  ...
  CORE_FIELD_NAMES = ['uri', 'vocabulary_string_key', 'value', 'authority', 'type', 'internal_id', 'uuid']
    ...
    self.handle_database_disconnect do
      ...
      unless current_tables.include?(UriService::TERMS)
        @db.create_table UriService::TERMS do |t|
          ...
          String :uri, text: true # This needs to be a text field because utf8 strings cannot be 2000 characters long in MySQL. uri_hash will be used to verify uniqueness.
          String :uri_hash, fixed: true, size: 64
          ...
          # we allow the same URI to be in more than one vocabulay, but it can
          # only appear once in each vocabulary
          index [:uri_hash, :vocabulary_string_key], unique: true
        end
  ...
  def create_term(type, opts)
    ...
    Retriable.retriable(on: [UriService::ExistingUriError, UriService::DuplicateUuidError], tries: 5, base_interval: 0.1) do
      ...
      if type == UriService::TermType::EXTERNAL
        ...
        return create_term_impl(type, vocabulary_string_key, value, uri, authority, uuid, additional_fields)
      else
        ...
        if type == UriService::TermType::TEMPORARY
          ...
          uri.path += uuid # UUID is used to generate local URI
          uri = uri.to_s
        end
        return create_term_impl(type, vocabulary_string_key, value, uri, authority, uuid, additional_fields)
      end
    ...
    raise UriService::DuplicateUuidError, "UriService generated a duplicate random UUID (via SecureRandom.uuid) too many times in a row.  Probabilistically, this should never happen."
  end
  ...
  def generate_frozen_term_hash(vocabulary_string_key, value, uri, authority, additional_fields, type, internal_id, uuid)
    hash_to_return = {}
  ...
  def create_term_solr_doc(vocabulary_string_key, value, uri, authority, additional_fields, type, internal_id, uuid)
    doc = {}
    doc['uri'] = uri
    doc['value'] = value
    doc['type'] = type
  ...
  def send_term_to_solr(vocabulary_string_key, value, uri, authority, additional_fields, type, internal_id, uuid, commit=true)
    doc = create_term_solr_doc(vocabulary_string_key, value, uri, authority, additional_fields, type, internal_id, uuid)
    @rsolr_pool.with do |rsolr|
      rsolr.add(doc)
  ...
  # Finds the term with the given internal id
  def find_term_by_internal_id(id)
    results = self.find_terms_where({internal_id: id}, 1)
    return results.length == 1 ? results.first : nil
  end

  # Finds the term with the given uuid
  def find_term_by_uuid(uuid)
    results = self.find_terms_where({uuid: uuid}, 1)
    return results.length == 1 ? results.first : nil
  end
  ...
  def term_solr_doc_to_frozen_term_hash(term_solr_doc)
    ...
    uuid = term_solr_doc.delete('uuid')

    return generate_frozen_term_hash(vocabulary_string_key, value, uri, authority, additional_fields, type, internal_id, uuid)
  end
  ...
  def update_term(uri, opts, merge_additional_fields=true)
    self.handle_database_disconnect do
      ...
      @db.transaction do
        ...
        self.send_term_to_solr(term_db_row[:vocabulary_string_key], new_value, uri, new_authority, new_additional_fields, term_db_row[:type], term_db_row[:id], term_db_row[:uuid])
      end

      return generate_frozen_term_hash(term_db_row[:vocabulary_string_key], new_value, uri, new_authority, new_additional_fields, term_db_row[:type], term_db_row[:id], term_db_row[:uuid])
    end
  end
  ...
  def create_term_impl(type, vocabulary_string_key, value, uri, authority, uuid, additional_fields)
    raise UriService::InvalidTermTypeError, 'Invalid type: ' + type unless VALID_TYPES.include?(type)
    ...
    self.handle_database_disconnect do
      ...
      @db.transaction do
        ...
        begin
          ...
          send_term_to_solr(vocabulary_string_key, value, uri, authority, additional_fields, type, db_id, uuid)
        rescue Sequel::UniqueConstraintViolation

          # Check if we minted a duplicate UUID (which is unlikely, due to probability)
          if @db[UriService::TERMS].where(uuid: uuid).count > 0
            raise UriService::DuplicateUuidError, 'Minted a duplicate UUID (which is Probabilistically very unlikely). Please try your operation again.'
          end
          ...
          raise UriService::ExistingUriError, "A term already exists in #{vocabulary_string_key} with uri: #{uri} (conflict found via uri_hash check)"
        end

        return generate_frozen_term_hash(vocabulary_string_key, value, uri, authority, additional_fields, type, db_id, uuid)
      end
    end
  end
...
class UriService::DuplicateUuidError < UriService::Error;end
